...
本 Skill 直接从既有飞书源表生成标准 `facts`，再由本地编译器和执行器完成计算。统一 runner 在取数前规范化时期，并基于实时结构索引和指标元信息（`可支持时间粒度`、`可支持拆解维度`、`聚合方式`）一次决定直接事实、细粒度聚合或注册派生路径；不经过竞品 `query.py` 的非标准结果转换，不调用其他 Skill 或小策 CLI。指标元信息中的 `指标公式`、`使用说明` 不参与事实能力判断或自动口径推断；只有【口径定义】作为源口径定义保留并在结果展示阶段按实际结果血缘投影。用户 Query 明确给出的归因公式仍按归因契约执行。
...
1. 读取统一分析请求契约。对用户显式独立问题分别保留不可变原始 Query，在 `/workspace/runtime` 的 task 目录运行 `scripts/select_query_policy.py`。Selector 对路由副本执行 Unicode NFKC、空白、大小写和路由标点归一化，但 `raw_query` 永不改写；规则索引由 active rule 的 `routing.terms` 编译生成并在发布时校验可达性。`no_match`、`fallback_raw`、超时、命令失败或输出不可解析时，本 task 禁止再次尝试 Policy，直接从原始 Query 生成 IR。
...
4. 只声明用户要求或通过 Query Policy 正常补充的业务指标、时期、范围、派生和归因目标。单个原生年、季、月、ISO 周写入 `analysis_task.periods`；上半年、截至目前或任意闭区间等非固有跨度写入 `analysis_task.period_requests`，保留起止日期，只有用户明确要求按年/季/月/周展示时才填写 `requested_grain`。派生与归因并存时分别保留各自时期角色：归因仍使用目标内 `periods`，不得为了复用事实而覆盖任务级派生时期。不要为标准指标组合补写基础指标，不要为时间跨度累计或逐期展示手写 `input_adaptations`、子时期角色或计算 AST；统一 runner 根据源表能力生成这些执行细节。
   开放结束词（如“今年截至目前、今年以来、截至最新数据”）可在 `period_requests` 标记 `end_semantics=latest_source_complete`、`upper_bound_source=current_date`；明确结束日期、固有周期和相对窗口不得标记。裸的注册组合家族词（如 `TR`）不指定具体组合，交由 Resolve 依据组合注册表澄清；完整组合名（如“广告支付TR”）按现有精准组合流程处理。事实指标不使用注册组合家族发现。
...
纯口径问题（明确询问指标定义、口径或包含范围，且不要求数值、趋势、派生或归因）可使用 `metric_definition_request/1.0`。该分支只复用同一 Gateway 的 Resolve 候选解析，在唯一绑定后读取【口径定义】并结束；不进入 Prepare、Compile、Fetch 或 Execute。分析结果后的口径追问优先使用上一轮 `answer_basis`，不重新取数；只有定义缺失时才按已绑定源指标重新 Resolve。带新时期或新数值要求的追问仍按普通分析处理。
