...
`aggregate_level` 的 binding 额外携带单个 `source_dimension`，能力投影必须保留该物理维度及当前 revision 枚举，但不得把成员复制进模型侧 resolve request。Provider 接口不新增方法；现有 resolved capabilities 的 `availability.metrics[*].members` 可选字段用于证明事实块成员覆盖，旧 Provider 未提供时仍由后续事实完整性闸门判断，不能默认缺失成员为零。
