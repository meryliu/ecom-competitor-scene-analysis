...
`produced_refs` 只在 action 带 `ir_effect_contract` 时必填。归因契约每项使用 `{"collection":"attribution_targets","id":"实际 target_id"}`；`scoped_expansion` 每项增加契约声明的语义 `role`。它只声明该 action 新增或拥有的目标，不得绑定用户显式目标或其他 action 的产物。同一目标最多由一个 action 绑定。校验器只修改契约明确拥有的字段：归因契约只补齐缺失或非协议枚举的 `target_semantics`；范围展开契约只规范声明维度的 `metric_constraints`，并通过 `analysis_task.metrics` 校验 role 对应的指标身份。其他时期、输出、范围和 requirement 字段均不修改。身份或合法协议字段冲突时返回 `fallback_raw`，不得覆盖。

## 规则到 IR

- 模糊指标表现：按目标作用域读取结构化语义帧。目标指标可由用户明确，也可由上游 Policy 仅展开指标集合；只要该作用域还没有指标值、同比、环比、份额或其他具体输出要求，且同一 task 无归因语义、用户无输出限制，就保留原模糊 `fact_observations`，并为同一指标、时期、范围、视图和拆解增加一个 `yoy_growth`。不得用 `analysis_goal` 自由文本推断输出槽位是否已满足。若上游规则已经给出完整输出需求，则不重复展开。新增项固定使用 `criticality=optional`、`default_output_role=performance_yoy_supplement`、`provenance=business_policy`。Resolve 先隔离该可选补充项解析主需求，再单独解析补充项；只有主结果与补充结果均为同一个源侧预计算派生事实，且输出语义、有效时期、筛选、拆解和视图完全相同时才去重。使用同一基础源指标计算事实值与同比不构成重复。Prepare 只允许静默删除上述三重标记完整匹配的补充项，并仅清理该补充项独占且无人继续引用的安全比较时期；显式同比及普通孤立时期仍按原校验报错。该补充项不参与原需求的候选选择；无法唯一执行时静默省略，不产生澄清、阻断、缺口披露或状态降级。“水平如何/水平怎么样”与“表现如何/表现怎么样”等价；“指标值/数值/金额/规模/数量/多少/水平值/具体水平”是显式绝对值，“同比/环比/增速/增长率/涨幅/变化”是显式比较，“只看/仅看/不需要同比”是输出限制，“口径/定义/怎么计算”是口径问题，均不应用本规则。task 只要包含归因目标、归因因子或“归因/原因/为什么/拆解/驱动/贡献/拉动/拖累/影响因素/下滑原因/增长原因”任一语义即整体排除，P0 不做子句级拆分。
...
- 京东佣金：单平台未指定口径时并列两个事实需求；通用佣金需求使用 `TOP6平台=京东`，指标名已内含京东和 3P 口径的需求不得再附加平台约束；多平台时不增加京东专属 3P 口径。该非对称归属由 action 的 `scoped_expansion` 契约约束，未声明该契约的其他展开保持原行为。
