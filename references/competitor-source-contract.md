...
指标元信息中的 `可支持时间粒度`、`可支持拆解维度`、`聚合方式` 是事实能力判断的唯一源表依据。索引分别规范化为 `supported_grains`、`dimensions` 和 `aggregation_mode`；事实表仍需证明具体指标块和周期存在。只有【口径定义】按原文索引为指标的 `definition`（并记录 `definition_source=指标元信息.口径定义`），供最终答案按实际结果血缘展示。`指标公式`、`使用说明` 不建索引，不参与自动能力判断或口径推断。
