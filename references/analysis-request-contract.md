...
纯口径请求是独立的 `metric_definition_request/1.0`，不生成可执行分析 IR：它仅携带原始 Query、指标词和显式筛选，复用 Gateway Resolve 后返回【口径定义】。若请求同时包含数值、时期、趋势、派生或归因要求，必须按本契约生成 `analysis_ir/1.0`，不能走纯口径分支。
