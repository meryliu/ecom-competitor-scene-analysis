...
单位与指标对象按 provenance 处理：`model_inferred` 不作硬过滤且不获得排序收益，冲突作为软诊断交给 Prepare 用源元信息纠正；`user_formula` 仍按推断处理；没有 `metric_object_evidence/unit_evidence` 的 `user_explicit` 自动降级为推断。只有 Query 可追溯证据、注册定义、业务意图规则或源元数据声明才是强约束。Compile、Execution 和事实质量闸门不放宽单位一致性与量级校验。维度唯一域推断复用当前请求的内存索引和规范化枚举集合，不增加 Provider 调用；公开候选和拒绝证据继续受策略上限约束，不把完整源索引放入模型上下文。
