...
注册组合采用“直接事实优先、组合回退”的两阶段决策。直接指标唯一可用时继续绑定直接事实；存在多个可行直接候选时仍须确认，不能用组合路径绕过歧义；只有直接候选集合为空且同一 `metric_ref` 命中注册组合时，通用解释 case 才标记为 `deferred`，由 Prepare 根据组合叶子状态决定继续编译或激活具体 `composition_input` case。没有注册组合的普通指标仍按原规则阻断。该延后只改变 case 激活时机，不放宽粒度、维度、可加性、单位、缺失值或执行质量校验。
